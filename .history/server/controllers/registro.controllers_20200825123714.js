...
const { getNroRegristro } = require("../controllers/solicitudesNro.controllers")
...
const crearRegistro = async(req, res = response, next) => {
    let ultimoNroRegistro = await Registro.findOne().sort({ field: 'asc', _id: -1 }).limit(1);

    if (ultimoNroRegistro) {
        req.body.data.nroRegistro = getNroRegistro(ultimoNroRegistro.nroRegistro);
    } else {
