...
const getSector = async(req, res = response) => {
    const sectores = await Sector.find().populate("usuario", "nombre img");
...
const getSectorById = async(req, res = response) => {
    ...
    try {
        const sector = await Sector.findById(id).populate("usuario", "nombre img");
        res.json({
            ok: true,
            sector,
        });
...
const modificarSector = async(req, res = response) => {
    ...
    try {
        ...
        const newSector = await Sector.findByIdAndUpdate(id, cambioSector, {
            new: true,
        });
        res.json({
            ok: true,
