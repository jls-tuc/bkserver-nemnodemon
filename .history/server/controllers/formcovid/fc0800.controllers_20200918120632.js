...
const getForm = async(req, res = response) => {
    const fechaInicial = req.body.fechaBusqueda; // ejemplo: '2020/09/18'
    console.log('inicio', req.body.fechaBusqueda)

    const fechaFinal = fechaInicial.substring(0, 8).concat(Number(fechaInicial.substring(8)) + 1);
    console.log("fech busqueda", fechaFinal)
    Fc0800.find({ $and: [{ fecha: { $gte: new Date(fechaInicial) } }, { fecha: { $lt: new Date(fechaFinal) } }] }, (err, reg) => {
        if (err) {
