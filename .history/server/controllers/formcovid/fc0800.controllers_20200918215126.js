...
const getForm = async (req, res = response) => {
  const fechaDesde = req.query.fechaDesde; // ejemplo: '2020/09/18'
  // console.log("desde", req.body.fechaDesde);
  const fechaHasta = req.query.fechaHasta;
  //console.log("hasta", req.body.fechaHasta);
  const start = fechaDesde
    .substring(0, 8)
    .concat(Number(fechaDesde.substring(8)));
  const end = fechaHasta
    .substring(0, 8)
    .concat(Number(fechaHasta.substring(8)) + 1);
  //console.log("fech busqueda", start);
  await Fc0800.find(
    { $and: [{ fecha: { $gte: new Date(start), $lt: new Date(end) } }] },
    (err, reg) => {
